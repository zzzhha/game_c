		...
		for (j = 0; j < COL; j++)
		{
			...
			if (j < COL - 4)//横着是否胜利
			{
				if (map[i][j] == map[i][j + 1] && map[i][j] == map[i][j + 2]
					&& map[i][j] == map[i][j + 3] && map[i][j] == map[i][j + 4]&&map[i][j]!=0)
				{
					gg = 1;
				}
			}
			if(i<ROW-4)//竖着是否胜利
			{
				if (map[i][j] == map[i + 1][j] && map[i][j] == map[i + 2][j]
					&& map[i][j] == map[i + 3][j] && map[i][j] == map[i + 4][j] && map[i][j] != 0)
				{
					gg = 1;
				}
			}
			if (i < ROW - 4 && j < COL - 4)//左上至右下是否胜利
			{
...
void gameView_ShowMap()//展示棋盘
{
	...
	for (i = 0; i < ROW; i++)
	{
		...
		for (j = 0; j < COL; j++)
		{
			if (i != ROW-1)//打印棋盘中的横杠，每两个数字中间一个横杠，最上和最下边没有
			{
				printf("___ ");
			}
		}
